...
class BindingViewModel : ViewModel() {
    val bindingData: List<ClassifiedServerList<BindingData>> = listOf(
        ClassifiedServerList(
            ...
            "皮肤", listOf(
                BindingData("变色", "exp1","详情查看 https://wiki.ddnet.org/index.php?title=Binds/zh&variant=zh-hans#%E5%8F%98%E8%89%B2Tee"),
            ...
            "其他", listOf(
                ...
                BindingData("发送特定表情", "bind [按键] emote 2","表情编号可查看 https://wiki.ddnet.org/index.php?title=Binds/zh&variant=zh-hans"),
                BindingData(
                    "八向射击",
                    "bind x \"+toggle cl_mouse_max_distance 2 400; +toggle inp_mousesens 2 164; +showhookcoll\""
                ),
                BindingData("显示所有玩家辅助线","bind s \"+showhookcoll; +toggle cl_show_hook_coll_other 2 1\"")
            )
        )
    )
}
